...
## Оглавление
- [Как построить надёжный ров?](#как-построить-надёжный-ров)
  - [Оглавление](#оглавление)
  - [Выбор провайдера](#выбор-провайдера)
  - [Настройка](#настройка)
    - [Начинаем с хоста](#начинаем-с-хоста)
    - [Установка 3x-ui](#установка-3x-ui)
    - [Настройка 3x-ui](#настройка-3x-ui)
...
### Что можно улучшить?
1. В этом гайде использовалась схема маскировки под чужой сайт, однако так же вы можете маскироваться и под свой, что порой увеличивает скрытность впн.
2. Для этого вы можете добавить домен первого уровня своему серверу, чтобы больше походить на естесвтенный сайт. Если не хочется покупать, можно найти бесплатный домен второго уровня
3. После добавления домена первого уровня появится возможность выдать себе бесплатно сертификаты от CloudFlare. Это круче, чем использовать самоподписанные.
4. Вынести сервис подписки на отдельных хост. Получится схема: `User -> Subscription Sevrer <-> VPN Servers`. Она позволит одной строкой для подключения (подпиской) выдавать несколько ВПН из разных локаций
5. Добавить промежуточный хост для обхода белых списков и удобной машрутизации (запросы на российские сервисы остаются в России, всё остальное — зарубеж). Получится схема: `User -> Russian VPN Sevrer <-> Foreign VPN Servers`.
6. Поиграться с протоколами и схемами проксирования. Вместо собственного промежуточного хоста можно попробовать использовать CDN сервера CloudFlare через WARP. И т.п.
